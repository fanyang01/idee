...
class BaseTool(ABC):
    ...
    def _get_schema_from_pydantic(self, model_class: Type["BaseModel"]) -> Dict[str, Any]:
        ...
        Assumes the latest Pydantic version (v2) is installed.
        """        
        # Get schema directly using model_json_schema method (Pydantic v2)
...
async def run_command(command: str, timeout: float = COMMAND_TIMEOUT_SECONDS) -> Tuple[str, str, int]:
    ...
    try:
        # Use asyncio.create_subprocess_shell for async handling
